...
to_plot <- subset(df, short_title %in% nine_titles)
to_plot$facet_order <- factor(to_plot$short_title, levels = c("The historical register,", "John Bull in his senses:", "The compleat housewife:", "The grave. A poem.", "Cadenus and Vanessa.", "Kent's directory", "The london songster; or polite musi", "Sketches from nature,", "The shopkeeper's and tradesman's as"))

#change name of book price in dataframe
to_plot$clean_size[to_plot$clean_size == 4] <- "quarto"
to_plot$clean_size[to_plot$clean_size == 8] <- "octavo"
to_plot$clean_size[to_plot$clean_size == 12] <- "duodecimo"

ggplot(to_plot, aes(x=as.numeric(as.character(year)),y=as.numeric(as.character(farthings_per_page)),colour=as.factor(clean_size))) +
  geom_jitter() +
